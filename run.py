...
def printGameResult(gameState):
    ...
    if '_' not in gameState["guessed_word"]:
        print(
            'You Won! The you guessed all the letters in the word ' +
            gameState["random_word"]
        )
        return
...
# Select a random word from the word list & convert all characters to uppercase
gameState["random_word"] = random.choice(hidden_word_list_level_one).upper()

# guessed_word represented by "_" is as long as the hidden random_word
